...
export default function RankTracker() {
    ...
    const handleAdd = async (e: React.SubmitEvent) => {
        ...
        setAddError("");
        try {
            const response = await api.post("/api/rank/add", {keyword: newKeyword.trim(), url: newUrl.trim()});
            if (response.data.success) {
                setKeywords((prev)=> [response.data.tracking, ...prev]);
                setNewKeyword("");
                setNewUrl("");
                setShowAddModal(false);
                const id = response.data.tracking._id;
                const pollInterval = setInterval(async()=>{
                    try {
                        const check = await api.get(`/api/rank/${id}`);
                        if(check.data.success && check.data.tracking.status !== "checking"){
                            clearInterval(pollInterval);
                            setKeywords((prev)=> prev.map((k)=> k._id === id ? check.data.tracking : k));
                        }
                    } catch (error: any) {
                        console.error("Error checking keyword ranking:", error.message);
                    }
                }, 3000)
            } else {
            ...
            setAddError(error.response.data.message || "Internal server error");
        }
        setAdding(false);
    };
    ...
    const handleRefresh = async (id: string) => {
        ...
        try {
            ...
            console.error("Error refreshing keyword:", error.message);
            setRefreshing(null);
        }
    };
    ...
    const handleDelete = async (id: string) => {
        ...
        setDeleting(id);
        try {
            const response = await api.delete(`/api/rank/keyword/${id}`);
            if (response.data.success) {
    ...
    const handleToggle = async (id: string) => {
        try {
            ...
            if (response.data.success) {
                setKeywords((prev) => prev.map((k) => (k._id === id ? {...k, active: response.data.tracking.active} : k)));
            }
        } catch (error: any) {
            console.error("Error deleting keyword:", error.message);
        }
